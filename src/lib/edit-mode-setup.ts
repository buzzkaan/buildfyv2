export async function setupEditModeInSandbox(sandbox: any) {
  try {
    // Get edit mode script content
    const editModeScript = `// Edit Mode Script - Injected into sandbox
(function() {
...
`;
    ...
    // Write edit-mode.js to public folder
    await sandbox.files.write('/home/user/public/edit-mode.js', editModeScript);
    console.log('✅ Edit mode script written to sandbox');

    // Read layout file
    const layoutContent = await sandbox.files.read('/home/user/app/layout.tsx');

    // Check if already added
    if (!layoutContent.includes('edit-mode.js')) {
      // Add <head> with script tag before <body> (Next.js 15 doesn't have <head> by default)
      let updatedLayout = layoutContent;

      if (layoutContent.includes('</head>')) {
        // Layout already has <head>, add script inside it
        updatedLayout = layoutContent.replace(
          '</head>',
          '  <script src="/edit-mode.js"></script>\n      </head>'
        );
      } else {
        // Layout doesn't have <head>, add it before <body>
        updatedLayout = layoutContent.replace(
          '<body',
          '<head>\n      <script src="/edit-mode.js"></script>\n    </head>\n    <body'
        );
      ...
      await sandbox.files.write('/home/user/app/layout.tsx', updatedLayout);
      console.log('✅ Edit mode script tag added to layout');
    } else {
      console.log('ℹ️ Edit mode already configured in layout');
    }
